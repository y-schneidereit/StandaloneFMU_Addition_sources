...
# define RT_MDL_TYPE      RT_MODEL_Addition_T
#endif
...
fmi2Component fmi2Instantiate(fmi2String instanceName, fmi2Type fmuType, fmi2String fmuGUID,
    ...
    if (!functions->logger) {
        ...
        return NULL;
    } else if (strcmp(fmuGUID, "{8f600e4d-a6f4-c2ed-3558-a9cbedce34b9}")) {
        functions->logger(functions->componentEnvironment, instanceName, fmi2Error, "error",
                "fmi2Instantiate: Wrong GUID %s. Expected %s.", fmuGUID, "{8f600e4d-a6f4-c2ed-3558-a9cbedce34b9}");
        return NULL;
    }
    ...
    modelData->rList[0] = &(Addition_U.x);
    modelData->rList[1] = &(Addition_U.y);
    modelData->rList[2] = &(Addition_Y.Ergebnis);
...
fmi2Status fmi2DoStep(fmi2Component c, fmi2Real currentCommunicationPoint,
    ...
    steps = (int)(communicationStepSize / (double)1 + 0.5);    
    modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
            fmi2OK, "info", "CommunicationStepSize=%.16g, LocalSolverStepSize=%.16g", communicationStepSize, (double)1);
    if (communicationStepSize < 0 || fabs(communicationStepSize - steps * (double)1) > 1.0E-6 * (double)1) {
        modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
                    fmi2Error, "error", "Stepsize must be non-negative and divisible by %.16g", (double)1);
        returnStatus = fmi2Error;
    } else if (fabs(modelData->time - currentCommunicationPoint) > 1.0E-6 * (double)1) {
        modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
